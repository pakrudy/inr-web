            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div>
                            @if ($recommendation->status === 'pending_renewal')
                                <a href="{{ route('customer.recommendations.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                                    {{ __('Kembali') }}
                                </a>
                                @if ($recommendation->is_awaiting_upgrade_payment)
                                    <a href="{{ route('customer.recommendations.payment.create', ['recommendation' => $recommendation, 'type' => 'upgrade']) }}" class="ml-3 inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                        {{ __('Bayar Upgrade') }}
                                    </a>
                                @elseif ($recommendation->status === 'active' && !$recommendation->is_indexed && !$recommendation->has_pending_upgrade_process)
                                    <a href="{{ route('customer.recommendations.upgrade.select', $recommendation) }}" class="ml-3 inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                        {{ __('Upgrade ke Terindeks') }}
                                    </a>
                                @elseif ($recommendation->status === 'expired' && !$recommendation->has_pending_renewal_payment)
                                    <a href="{{ route('customer.recommendations.payment.create', ['recommendation' => $recommendation, 'type' => 'renewal_r1']) }}" class="ml-3 inline-flex items-center px-4 py-2 bg-orange-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-orange-700 focus:bg-orange-700 active:bg-orange-900 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        <dl class="divide-y divide-gray-200">
                            ...
                            <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                <dt class="text-sm font-medium leading-6 text-gray-900">{{ __('Kategori') }}</dt>
                            ...
                            <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                <dt class="text-sm font-medium leading-6 text-gray-900">{{ __('Peta Lokasi') }}</dt>
                            ...
                            <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                    @if ($recommendation->status === 'pending_renewal')
                                        <span class="text-yellow-800">{{ __('Menunggu Persetujuan Perpanjangan') }}</span>
                                    @elseif ($recommendation->status === 'pending' && $recommendation->has_pending_initial_payment)
                                        <span class="text-yellow-800">{{ __('Menunggu Persetujuan Aktivasi') }}</span>
                                    @else
                                    ...
                                    @elseif ($recommendation->is_awaiting_upgrade_payment)
                                        <span class="text-blue-800">{{ __('Waiting Payment') }}</span>
                                            @elseif ($recommendation->has_pending_upgrade_process)
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">{{ __('Waiting Approval') }}</span>                                    @else
