            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900">
                    ...
                    <p class="mt-2 text-gray-600">
                        Anda mengajukan upgrade untuk legacy <span class="font-semibold">"{{ $legacy->title }}"</span> dengan <span class="font-semibold">{{ $package->name }}</span>.
                    </p>
                    <p class="text-gray-600">
                        Mohon lengkapi data di bawah ini untuk verifikasi oleh tim kami.
                    </p>

                    <form method="POST" action="{{ route('customer.legacies.upgrade.apply', ['legacy' => $legacy, 'package_slug' => $package->slug]) }}" class="mt-8 space-y-6">
                        @csrf
                        ...
                        <div class="flex items-center gap-4 pt-4 border-t border-gray-200">
                            ...
                            <a href="{{ route('customer.legacies.upgrade.select', $legacy) }}" class="text-gray-600 hover:text-gray-900">{{ __('Batal') }}</a>
                        </div>
                    </form>
                </div>
            </div>
