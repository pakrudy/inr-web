                ...
                <div class="p-8 text-gray-900 space-y-6">
                    <div>
                        ...
                        <p class="mt-1 text-gray-600">
                            Pengajuan untuk legacy <span class="font-semibold">"{{ $application->legacy?->title }}"</span> oleh <span class="font-semibold">{{ $application->user?->name }}</span>.
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
                        @if ($application->status === 'pending')
                            <form method="POST" action="{{ route('admin.legacy-upgrades.reject', $application) }}" onsubmit="return confirm('Are you sure you want to reject this application?');">
                                @csrf
                                <x-danger-button type="submit">{{ __('Tolak') }}</x-danger-button>
                            </form>
                            <form method="POST" action="{{ route('admin.legacy-upgrades.approve', $application) }}" onsubmit="return confirm('Are you sure you want to approve this application? The user will be notified to make a payment.');">
                                @csrf
                        ...
                        @endif
                        <a href="{{ route('admin.legacy-upgrades.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                            {{ __('Kembali') }}
                        </a>
                    </div>
                </div>
